// KyMonBanHaDo.js - Hà Đồ Kỳ Môn Độn Giáp Engine (BẢN HOÀN CHỈNH)
// Áp dụng Cửu Cung Hà Đồ: Khôn=ĐN, Tốn=TN, Ly=7, Đoài=9
// Di Cung Bát Thần/Bát Môn/Cửu Tinh xoay vòng chuẩn VONG_QUAY Hà Đồ
// Giữ nguyên 100% các thông số phụ trợ: Trường sinh, Quý nhân, Thái tuế...
...
(function (root, factory) {
  ...
  const TUAN_THU_MAP = {
    'Giáp Tý':'Mậu', 'Giáp Tuất':'Kỷ', 'Giáp Thân':'Canh',
    'Giáp Ngọ':'Tân', 'Giáp Thìn':'Nhâm', 'Giáp Dần':'Quý'
  };
  ...
  // ============================================================
  // 3. CÁC HÀM XỬ LÝ KỲ MÔN (ĐỘNG CƠ HÀ ĐỒ)
  // ============================================================
  ...
  function timCungGocTrucPhu(db, tuanThu) {
    let canTuanThu = TUAN_THU_MAP[tuanThu] || 'Mậu';
    for (let i = 1; i <= 9; i++) if (db[i] === canTuanThu) return i;
    return 1;
  }

  function tinhThienBanVaCan(db, cungGoc, canGio, tuanThu) {
    let canGioThucTe = canGio === 'Giáp' ? TUAN_THU_MAP[tuanThu] : canGio;
    let cungDich = 5;
  ...
  // ============================================================
  // 4. CÁC HÀM LOGIC PHONG THỦY BỔ TRỢ (ĐẦY ĐỦ NHƯ BẢN V1)
  // ============================================================
  ...
  function lapBanKyMon(dateStr, options = {}) {
    ...
    // 1. TÍNH TOÁN THIÊN VĂN (SỬ DỤNG 3META)
    ...
    const [canNam, chiNam] = lunar.getYearInGanZhiExact().split(' ');
    const [canThang, chiThang] = lunar.getMonthInGanZhiExact().split(' ');
    const [canNgay, chiNgay] = lunar.getDayInGanZhiExact().split(' ');
    const [canGio, chiGio] = lunar.getTimeInGanZhi().split(' ');

    let tkTen = lunar.getPrevJieQi(!1).getName();
    // Fix map tên Hán Việt nếu thư viện trả về sai
    ...
    // 2. LẬP BÀN ENGINE (HÀ ĐỒ VONG QUAY)
    const diaBan = tinhDiaBan(soCuc, isDuong);
    const cungGocTrucPhu = timCungGocTrucPhu(diaBan, tuanThu);
    const { thienBan, thienCanBan, cungDichSao } = tinhThienBanVaCan(diaBan, cungGocTrucPhu, canGio, tuanThu);
    const batMon = tinhBatMon(cungGocTrucPhu, isDuong, chiGio, chiNgay); // chiNgay là chi của Tuần Thủ
    const batThan = tinhBatThan(cungDichSao, isDuong);
    const anCan = tinhAnCan(diaBan);
    
    // 3. XÁC ĐỊNH TRỰC PHÙ, TRỰC SỬ
    const zhiFu = {
    ...
    for (let c = 1; c <= 9; c++) {
      ...
      palaces.push({
        ...
        // Thêm các thông số bổ trợ của V1
    ...
    return {
      ...
      tuanThu: { ten: tuanThu, khongVong, tamNguyen },
